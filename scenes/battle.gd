...
func display_text(panel_node: Panel, text: String) -> void:
	panel_node.show()
...
func set_health(hpbar_node, current_health: float, max_health: float) -> void:
...
func _on_player_switch_selected(index: int) -> void:
	...
	match current_state:
		...
		BattleState.RESOLVE_ATTACKS:
			if selectedPlayer != newPlayerCharacter:
				...
				if selectedEnemy.status.name == "Confused" and randf() < 0.4:
					...
					await enemy_attack.execute(
						selectedEnemy, selectedPlayer, enemy_anim, player_anim, soundfx_player,
						$Textbox, %PlayerHPBar, get_tree(), $PlayerContainer/StatusBox
					)
					await get_tree().create_timer(1.3).timeout
				...
				if selectedPlayer.status.name == "Confused" and randf() < 0.4:
					...
					await get_tree().create_timer(1.0).timeout
					await player_attack.execute(
						selectedPlayer, selectedEnemy, player_anim, enemy_anim, soundfx_player,
						$Textbox, %EnemyHPBar, get_tree(), $EnemyContainer/StatusBox
					)
					await get_tree().create_timer(1.3).timeout
